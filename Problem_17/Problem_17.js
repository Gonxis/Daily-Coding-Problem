...
/**
 * Find the length of the longest absolute path to a file as string.
...
let longestPath = (string) => {
    ...
    string.split('\n').forEach(dirFile => {
        let level = dirFile.lastIndexOf('\t') + 1;
        while (level + 1 < stack.length) stack.pop();
        let len = stack[stack.length - 1] + dirFile.length - level + 1;
        stack.push(len);
        if (dirFile.includes('.') && len - 1 > maxLen) maxLen = len - 1;
    });
